...
function loadTableData(tableName) {
    fetch(`${options}/api/v1/${tableName}`)
        .then(response => response.json())
        .then(data => {
            var metadata = fetch(`${options}/api/v1/${tableName}/metadata`)
            .then(response => response.json())
            .then(data => {return data})
            const tableData = document.getElementById("table-data");
            const tableNameElement = document.getElementById("table-name");
            tableNameElement.textContent = tableName.replace(/\b\w/g, char => char.toUpperCase());
            ...
            // Заголовки на основе ключей первого объекта
